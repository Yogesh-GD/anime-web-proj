...
export default function Recommandation({ animekey }) {
    ...
    useEffect(() => {
        const fetchdata = async () => {
            try {
                setLoad(false);
                const data = await axios.get(`https://api.jikan.moe/v4/manga/${animekey}/recommendations`)
                    ...
                    <div className="recommandationlist">
                        {
                            recommandationData.map((data, index) => {
                                return (
                                        ...
                                        <div className="recommand" key={index}>
                                            <NavLink to={`/MangaDetail/${data.entry.mal_id}`} style={theme ? { color: '#36BCE5' } : { color: "#E53637" }} onClick={() => { localStorage.setItem('animeDetailId', data.entry.mal_id) }} >
                                            ...
                                            <div className="cardname">
                                                <NavLink to={`/MangaDetail/${data.entry.mal_id}`} style={theme ? { color: '#36BCE5' } : { color: "#E53637" }}  > {data.entry.title}</NavLink>
                                            </div>
                                        </div>
